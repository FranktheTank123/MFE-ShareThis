...
s3_save_path = "s3n://log.sharethis.com/SentimentAverage_v2/"+ region + "/"
...
def getSentimentSummary_bin2(labeled_sharethis_json_final, curr_date):
	"""
	here we set look back to x min and do a rolling mean sentiment without considering the
	trading period
	"""
	total_results = getEmptySparkDf()

	# labeled_sharethis_json_final.cache() # cache first
 	# first we create the time of each day's start/end trading period
	start_time = datetime(curr_date.year, curr_date.month, curr_date.day, 0, 0, 0)
	# then we create a trading time list
	timelist = [ (start_time + timedelta(minutes=x*15)) for x in range(96)]
	for i, each_time in enumerate(timelist):
	    # print("Now Processing:", each_time)
	    each_time_end = (each_time + timedelta(minutes = 15)) 
	    # create a function to filter in the period: 
	    # [ t-look_back, t ]
	    if i == 95:
	    	getTime = udf(lambda x: x>= each_time, BooleanType()) ## edge case
	    else:
	    	getTime = udf(lambda x: x < each_time_end and x >= each_time, BooleanType())
	    # only save the current timeframe's data	    
	    _temp = (labeled_sharethis_json_final.filter(getTime("TimeStamp"))
	    			.groupby("Ticker")
		            .agg(mean("company_sentiment_score").alias("sentiment_mean"),
...
def getEmptySparkDf():
	"""
	Create an empty pyspark datafram
	"""
	schema = StructType([
...
def main():
	...
	while curr_date < dayend:
		...
		try:
			...
			total_results = getSentimentSummary_bin2(labeled_sharethis_json_final, curr_date)
